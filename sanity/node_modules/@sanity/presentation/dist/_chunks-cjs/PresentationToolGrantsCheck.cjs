...
const SharedStateProvider = function(props) {
  const { comlink: comlink2, children } = props, sharedState = react.useRef({});
  react.useEffect(() => comlink2?.on("visual-editing/shared-state", () => ({ state: sharedState.current })), [comlink2]);
  const setValue = react.useCallback(
    (key, value) => {
      sharedState.current[key] = value, comlink2?.post({ type: "presentation/shared-state", data: { key, value } });
    },
    [comlink2]
  ), removeValue = react.useCallback(
    (key) => {
      comlink2?.post({ type: "presentation/shared-state", data: { key } }), delete sharedState.current[key];
    },
    [comlink2]
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(index.SharedStateContext.Provider, { value: context, children });
...
}, IFrameElement = framerMotion.motion(styledComponents.styled.iframe`
...
const PERSPECTIVE_TITLE_KEY = {
  ...
  const {
  ...
  } = props, { t } = sanity.useTranslation(index.presentationLocaleNamespace), toggleViewportSize = react.useCallback(
    ...
    onRefresh(() => {
      iframeRef.current && (dispatch({ type: index.ACTION_IFRAME_RELOAD }), iframeRef.current.src = `${targetOrigin}${previewUrl || "/"}`);
    });
  }, [dispatch, onRefresh, previewUrl, targetOrigin, iframeRef]), previewLocationRoute = react.useMemo(() => {
    ...
    /* @__PURE__ */ jsxRuntime.jsx(
      ui.Tooltip,
      {
        ...
        content: /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { align: "center", style: { whiteSpace: "nowrap" }, children: [
          ...
          /* @__PURE__ */ jsxRuntime.jsx(ui.Box, { paddingY: 1, children: /* @__PURE__ */ jsxRuntime.jsx(ui.Hotkeys, { keys: ["Alt"], style: { marginTop: -4, marginBottom: -4 } }) })
        ] }),
        fallbackPlacements: ["bottom-start"],
        padding: 1,
        placement: "bottom",
  ...
  const renderDefault = react.useCallback((props2) => react.createElement(PreviewHeaderDefault, props2), []), header = props.options?.component ? react.createElement(props.options.component, { ...props, renderDefault }) : renderDefault(props);
  return /* @__PURE__ */ jsxRuntime.jsx(ui.Card, { flex: "none", padding: 2, shadow: 1, style: { position: "relative" }, children: /* @__PURE__ */ jsxRuntime.jsx(ui.Flex, { align: "center", style: { minHeight: 0 }, children: header }) });
};
function usePresentationPreviewHeader(props) {
  return react.useCallback(() => /* @__PURE__ */ jsxRuntime.jsx(PreviewHeader, { ...props }), [props]);
}
const MotionFlex = framerMotion.motion(ui.Flex), Preview = react.memo(
  react.forwardRef(function(props, forwardedRef) {
    const { dispatch, iframe, header, initialUrl, loadersConnection, overlaysConnection, viewport } = props, { t } = sanity.useTranslation(index.presentationLocaleNamespace), { devMode } = index.usePresentationTool(), prefersReducedMotion = ui.usePrefersReducedMotion(), ref = react.useRef(null), PreviewHeader2 = usePresentationPreviewHeader({
      ...props,
      iframeRef: ref,
      options: header
    });
    ...
    const loading = iframe.status === "loading" || iframe.status === "reloading", [timedOut, setTimedOut] = react.useState(!1), refreshing = iframe.status === "refreshing", [somethingIsWrong, setSomethingIsWrong] = react.useState(!1), iframeIsBusy = loading || refreshing || overlaysConnection === "connecting", handleRetry = react.useCallback(() => {
      ref.current && (ref.current.src = initialUrl.toString(), dispatch({ type: index.ACTION_IFRAME_RELOAD }));
    ...
    const onIFrameLoad = react.useCallback(() => {
      dispatch({ type: index.ACTION_IFRAME_LOADED });
    }, [dispatch]);
    react.useEffect(() => {
      if (!ref.current)
        return;
...
const usePopups = (controller) => {
  const [popups, setPopups] = react.useState(() => /* @__PURE__ */ new Set()), open = react.useCallback((url) => {
...
function PresentationTool(props) {
  const { canCreateUrlPreviewSecrets, canToggleSharePreviewAccess, canUseSharedPreviewAccess, tool } = props, components = tool.options?.components, _previewUrl = tool.options?.previewUrl, name = tool.name || index.DEFAULT_TOOL_NAME, { unstable_navigator, unstable_header } = components || {}, { navigate: routerNavigate, state: routerState } = router.useRouter(), routerSearchParams = sanity.useUnique(Object.fromEntries(routerState._searchParams || [])), initialPreviewUrl = usePreviewUrl(
    _previewUrl || "/",
    name,
    routerSearchParams.perspective === "published" ? "published" : "previewDrafts",
    routerSearchParams.preview || null,
    canCreateUrlPreviewSecrets
  ), canSharePreviewAccess = react.useMemo(() => _previewUrl && typeof _previewUrl == "object" && "draftMode" in _previewUrl && _previewUrl.draftMode ? (console.warn("previewUrl.draftMode is deprecated, use previewUrl.previewMode instead"), _previewUrl.draftMode.shareAccess !== !1) : _previewUrl && typeof _previewUrl == "object" && "previewMode" in _previewUrl && _previewUrl.previewMode ? _previewUrl.previewMode.shareAccess !== !1 : !1, [_previewUrl]), [devMode] = react.useState(() => {
    ...
    routerSearchParams,
    frameStateRef
  }), navigate = react.useMemo(() => debounce(_navigate, 50), [_navigate]), [state, dispatch] = react.useReducer(index.presentationReducer, {}, index.presentationReducerInit), perspective = react.useMemo(
  ...
  react.useEffect(() => {
  ...
  }, [targetOrigin, isLoading]), react.useEffect(() => {
    ...
    comlink$1.on("visual-editing/focus", (data) => {
      ...
      data.source === "manual" ? clearTimeout(refreshRef.current) : data.source === "mutation" && dispatch({ type: index.ACTION_IFRAME_REFRESH });
  ...
  const refreshRef = react.useRef(), handleRefresh = react.useCallback(
    (fallback) => {
      if (dispatch({ type: index.ACTION_IFRAME_REFRESH }), visualEditingComlink) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    /* @__PURE__ */ jsxRuntime.jsx(
      PresentationProvider,
      {
        ...
        searchParams,
        structureParams,
        children: /* @__PURE__ */ jsxRuntime.jsx(PresentationNavigateProvider, { navigate, children: /* @__PURE__ */ jsxRuntime.jsx(PresentationParamsProvider, { params, children: /* @__PURE__ */ jsxRuntime.jsx(SharedStateProvider, { comlink: visualEditingComlink, children: /* @__PURE__ */ jsxRuntime.jsx(Container, { height: "fill", children: /* @__PURE__ */ jsxRuntime.jsxs(Panels, { children: [
