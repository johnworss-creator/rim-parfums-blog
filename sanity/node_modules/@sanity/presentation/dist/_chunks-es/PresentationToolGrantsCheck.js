...
import { Button, Card, Flex, Container as Container$1, Stack, Text, Inline, Box, Code, Label, ErrorBoundary, Tooltip, TextInput, Spinner, useToast, MenuButton, Menu, Grid, Switch, MenuDivider, MenuItem, Hotkeys, usePrefersReducedMotion, TooltipDelayGroupProvider, BoundaryElementProvider } from "@sanity/ui";
import { uuid } from "@sanity/uuid";
import { useRef, useEffect, useCallback, useMemo, createContext, useContext, useLayoutEffect, useState, forwardRef, Suspense, memo, lazy, createElement, useImperativeHandle, startTransition, useReducer } from "react";
import { useTranslation, useUnique, getPublishedId, useDocumentPreviewStore, getPreviewStateObservable, useSchema, SanityDefaultPreview, getPreviewValueWithFallback, PreviewCard, Translate, CommentsIntentProvider, useActiveWorkspace, useClient, useCurrentUser, useEditState, useProjectId, useDataset, useWorkspace, useGrantsStore } from "sanity";
import { optional, object, pipe, minLength, string, record, unknown, SharedStateContext, presentationLocaleNamespace, usePresentationTool, PresentationSpinner, PresentationNavigateContext, PresentationParamsContext, PresentationContext, API_VERSION, ACTION_IFRAME_RELOAD, MAX_TIME_TO_OVERLAYS_CONNECTION, ACTION_IFRAME_LOADED, POPUP_CHECK_INTERVAL, presentationReducer, presentationReducerInit, ACTION_VISUAL_EDITING_OVERLAYS_TOGGLE, ACTION_IFRAME_REFRESH, COMMENTS_INSPECTOR_NAME, EDIT_INTENT_MODE, DEFAULT_TOOL_NAME } from "./index.js";
...
const SharedStateProvider = function(props) {
  const { comlink, children } = props, sharedState = useRef({});
  useEffect(() => comlink?.on("visual-editing/shared-state", () => ({ state: sharedState.current })), [comlink]);
  const setValue = useCallback(
    (key, value) => {
      sharedState.current[key] = value, comlink?.post({ type: "presentation/shared-state", data: { key, value } });
    },
    [comlink]
  ), removeValue = useCallback(
    (key) => {
      comlink?.post({ type: "presentation/shared-state", data: { key } }), delete sharedState.current[key];
    },
    [comlink]
  ), context = useMemo(
  ...
  return /* @__PURE__ */ jsx(SharedStateContext.Provider, { value: context, children });
}, PanelsContext = createContext(null);
...
}, IFrameElement = motion(styled.iframe`
...
const PERSPECTIVE_TITLE_KEY = {
  ...
  const {
  ...
  } = props, { t } = useTranslation(presentationLocaleNamespace), toggleViewportSize = useCallback(
    ...
    onRefresh(() => {
      iframeRef.current && (dispatch({ type: ACTION_IFRAME_RELOAD }), iframeRef.current.src = `${targetOrigin}${previewUrl || "/"}`);
    });
  }, [dispatch, onRefresh, previewUrl, targetOrigin, iframeRef]), previewLocationRoute = useMemo(() => {
  ...
  const renderDefault = useCallback((props2) => createElement(PreviewHeaderDefault, props2), []), header = props.options?.component ? createElement(props.options.component, { ...props, renderDefault }) : renderDefault(props);
  return /* @__PURE__ */ jsx(Card, { flex: "none", padding: 2, shadow: 1, style: { position: "relative" }, children: /* @__PURE__ */ jsx(Flex, { align: "center", style: { minHeight: 0 }, children: header }) });
};
function usePresentationPreviewHeader(props) {
  return useCallback(() => /* @__PURE__ */ jsx(PreviewHeader, { ...props }), [props]);
}
const MotionFlex = motion(Flex), Preview = memo(
  forwardRef(function(props, forwardedRef) {
    const { dispatch, iframe, header, initialUrl, loadersConnection, overlaysConnection, viewport } = props, { t } = useTranslation(presentationLocaleNamespace), { devMode } = usePresentationTool(), prefersReducedMotion = usePrefersReducedMotion(), ref = useRef(null), PreviewHeader2 = usePresentationPreviewHeader({
      ...props,
      iframeRef: ref,
      options: header
    });
...
const usePopups = (controller) => {
  const [popups, setPopups] = useState(() => /* @__PURE__ */ new Set()), open = useCallback((url) => {
  ...
  return useEffect(() => {
    ...
    if (popups.size) {
      const interval = setInterval(() => {
        ...
        for (const source of popups)
          source && "closed" in source && source.closed && closed.add(source);
        closed.size && setPopups((prev) => {
...
function useStatus() {
  const [statusMap, setStatusMap] = useState(
    ...
    if (values.find(({ status: status2 }) => status2 === "connected"))
      return "connected";
    const handshaking = values.filter(({ status: status2 }) => status2 === "handshaking");
    return handshaking.length ? handshaking.some(({ hasConnected }) => !hasConnected) ? "connecting" : "reconnecting" : "idle";
...
function PresentationTool(props) {
  const { canCreateUrlPreviewSecrets, canToggleSharePreviewAccess, canUseSharedPreviewAccess, tool } = props, components = tool.options?.components, _previewUrl = tool.options?.previewUrl, name = tool.name || DEFAULT_TOOL_NAME, { unstable_navigator, unstable_header } = components || {}, { navigate: routerNavigate, state: routerState } = useRouter(), routerSearchParams = useUnique(Object.fromEntries(routerState._searchParams || [])), initialPreviewUrl = usePreviewUrl(
    _previewUrl || "/",
    name,
    routerSearchParams.perspective === "published" ? "published" : "previewDrafts",
    routerSearchParams.preview || null,
    canCreateUrlPreviewSecrets
  ), canSharePreviewAccess = useMemo(() => _previewUrl && typeof _previewUrl == "object" && "draftMode" in _previewUrl && _previewUrl.draftMode ? (console.warn("previewUrl.draftMode is deprecated, use previewUrl.previewMode instead"), _previewUrl.draftMode.shareAccess !== !1) : _previewUrl && typeof _previewUrl == "object" && "previewMode" in _previewUrl && _previewUrl.previewMode ? _previewUrl.previewMode.shareAccess !== !1 : !1, [_previewUrl]), [devMode] = useState(() => {
  ...
  }), [overlaysConnection, setOverlaysConnection] = useStatus(), [loadersConnection, setLoadersConnection] = useStatus(), [previewKitConnection, setPreviewKitConnection] = useStatus(), { open: handleOpenPopup } = usePopups(controller), isLoading = state.iframe.status === "loading";
  useEffect(() => {
  ...
  }, [targetOrigin, isLoading]), useEffect(() => {
  ...
  const refreshRef = useRef(), handleRefresh = useCallback(
    (fallback) => {
      if (dispatch({ type: ACTION_IFRAME_REFRESH }), visualEditingComlink) {
        refreshRef.current = window.setTimeout(fallback, 300), visualEditingComlink.post({
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx(
      PresentationProvider,
      {
        ...
        searchParams,
        structureParams,
        children: /* @__PURE__ */ jsx(PresentationNavigateProvider, { navigate, children: /* @__PURE__ */ jsx(PresentationParamsProvider, { params, children: /* @__PURE__ */ jsx(SharedStateProvider, { comlink: visualEditingComlink, children: /* @__PURE__ */ jsx(Container, { height: "fill", children: /* @__PURE__ */ jsxs(Panels, { children: [
